 ...
 *
 * Abstract:
 *      The ni_modelframework.c file implements the VeriStand Model Framework interface 
...
/*
 * VeriStand Model Framework API version
 ...
 *
 * Abstract:
 *	Returns the version of the VeriStand Model Framework
 *
 * Output Parameters
...
DLL_EXPORT int32_t NIRT_ModelError(char* Errmsg, int32_t* msglen)
{
	...
	if (NIRT_system.errmsg != NULL)
	{
		...
		if (*msglen > 0) 
		{
			...
			{
				/* Get error message */
				*msglen = (int32_t)strlen(NIRT_system.errmsg);
			}
	...
	else if(NIRT_system.stopExecutionFlag == 1)
	{
		...
		if (*msglen > 0) 
		{
			...
			{
				/* Get error message */
				*msglen = (int32_t)strlen(simStoppedMsg);
			}
	...
	if ( (sigidx >= 0) && (sigidx < SignalSize) ) 
	{
		...
		if (blkname != NULL) 
		{
			/* no need for return string to be null terminated! */
				...
				{
					*bnlen = (int32_t)strlen(rtSignalAttribs[sigidx].blockname);
				}
		...
		if (signame != NULL) 
			{
				...
				{
					*snlen = (int32_t)strlen(rtSignalAttribs[sigidx].signalname);
				}
...
DLL_EXPORT int32_t NIRT_GetParameterSpec(int32_t* pidx, char* ID, int32_t* ID_len, char* paramname, int32_t *pnlen, int32_t *dattype, int32_t* dims, int32_t* numdim)
{
	...
	if ( (paramidx >= 0) && (paramidx < ParameterSize) ) 
	{
		if(ID != NULL) 
		{
			...
			{
				*ID_len = (int32_t)strlen(rtParamAttribs[paramidx].paramname);
			}
		...
		if(paramname != NULL) 
		{
			...
			{
				*pnlen = (int32_t)strlen(rtParamAttribs[paramidx].paramname);
			}
...
DLL_EXPORT int32_t NIRT_GetExtIOSpec(int32_t index, int32_t *idx, char* name, int32_t* tid, int32_t *type, int32_t *dims, int32_t* numdims)
{
	...
	{
		/* Get the port name */
    	int32_t sz = (int32_t)strlen(name);
...
DLL_EXPORT int32_t NIRT_GetErrorMessageLength(void)
{
	...
	if (NIRT_system.errmsg != NULL)
	{
		retval = (int32_t)strlen(NIRT_system.errmsg); 
	}
	else
	{
